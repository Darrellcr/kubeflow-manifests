...
# 01. Replace the namesapce of AuthorizationPolicy principal
    ...
    principals:
    - "cluster.pakcarik/ns/development/sa/default-editor"

# 02. Replace the namespace of node-ip-address of headGroupSpec and workerGroupSpec

    node-ip-address: $(hostname -I | tr -d ' ' | sed 's/\./-/g').raycluster-istio-headless-svc.development.svc.cluster.pakcarik
```
...
-   Connect to RayCluster via Ray client.
    ...
    import ray
    # For other namespaces use ray://${RAYCLUSTER_HEAD_SVC}.${NAMESPACE}.svc.cluster.pakcarik:${RAY_CLIENT_PORT}
